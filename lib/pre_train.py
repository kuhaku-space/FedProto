...
def test(args: Any, num_epoch: int, testloader: DataLoader) -> None:
    print("Start Testing")
    # model = Lenet(args=args)
    # model.to(args.device)
...
if __name__ == "__main__":
    ...
    train_dataset = datasets.CIFAR10(
        data_dir, train=True, download=False, transform=trans_cifar10_train
    )
    test_dataset = datasets.CIFAR10(
        data_dir, train=False, download=False, transform=trans_cifar10_val
    )
    trainloader = torch.utils.data.DataLoader(
        train_dataset, batch_size=100, shuffle=True, num_workers=2
    )
    testloader = torch.utils.data.DataLoader(
        test_dataset, batch_size=50, shuffle=False, num_workers=2
    )

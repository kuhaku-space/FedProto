...
class LocalUpdate(object):
    def __init__(self, args: Any, dataset: Dataset, idxs: set[int]):
        self.args = args
        self.trainloader = self.train_val_test(dataset, list(idxs))
        self.device = args.device
        self.criterion = nn.NLLLoss().to(self.device)

    def train_val_test(self, dataset: Dataset, idxs: list[int]) -> DataLoader:
        """
...
def test_inference_new(
    args, local_model_list, test_dataset, classes_list, global_protos=[]
):
    """Returns the test accuracy and loss."""
    loss, total, correct = 0.0, 0.0, 0.0
...
def test_inference_new_het(args: Any, local_model_list: list[nn.Module], test_dataset: Dataset, global_protos: dict[Any, Any] = {}) -> float:
    ...
    for batch_idx, (images, labels) in enumerate(testloader):
        ...
        a_large_num = 100
        outputs = a_large_num * torch.ones(size=(images.shape[0], 10)).to(
            device
        )  # outputs 64*10
...
def test_inference_new_het_lt(
    ...
    for idx in range(args.num_users):
        ...
        testloader = DataLoader(
            DatasetSplit(test_dataset, user_groups_gt[idx]), batch_size=64, shuffle=True
        )
        ...
        if global_protos != []:
            for batch_idx, (images, labels) in enumerate(testloader):
                ...
                ).to(device)  # initialize a distance matrix
                for i in range(images.shape[0]):
...
def test_inference_new_het_cifar(
    ...
    for batch_idx, (images, labels) in enumerate(testloader):
        ...
        a_large_num = 1000
        outputs = a_large_num * torch.ones(size=(images.shape[0], 100)).to(
            device
        )  # outputs 64*10
        for i in range(images.shape[0]):
            for j in range(100):
